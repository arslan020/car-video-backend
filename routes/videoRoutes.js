...
// @desc    Get a one-time direct upload URL from Cloudflare Stream
// @route   POST /api/videos/direct-upload-url
// @access  Private/Staff
router.post('/direct-upload-url', protect, async (req, res) => {
    try {
        ...
        // This URL lets the browser upload directly — no file passes through our server
        const response = await axios.post(
            `https://api.cloudflare.com/client/v4/accounts/${accountId}/stream/direct_upload`,
            {
                maxDurationSeconds: 3600, // 1 hour max
                requireSignedURLs: false,
                meta: { name: title || 'Car Video' }
            },
            {
                headers: {
        ...
        res.json({ uploadUrl: uploadURL, videoId: uid });

    } catch (error) {
        console.error('Direct upload URL error:', error.response?.data || error.message);
        res.status(500).json({ message: 'Failed to get upload URL', error: error.message });
    }
});

// @desc    Confirm upload and save video record to DB
// @route   POST /api/videos/confirm-upload
// @access  Private/Staff
router.post('/confirm-upload', protect, async (req, res) => {
    try {
        const accountId = process.env.CLOUDFLARE_ACCOUNT_ID;
        const apiToken = process.env.CLOUDFLARE_API_TOKEN;
        const { videoId, title, registration, make, model, vehicleDetails, mileage, reserveCarLink } = req.body;

        if (!videoId) {
            return res.status(400).json({ message: 'videoId is required' });
        }

        // Fetch video details from Cloudflare to get the thumbnail and confirm it exists
        let thumbnail = null;
        try {
            await new Promise(r => setTimeout(r, 1000)); // Let Cloudflare process
            const cfRes = await axios.get(
                `https://api.cloudflare.com/client/v4/accounts/${accountId}/stream/${videoId}`,
                { headers: { Authorization: `Bearer ${apiToken}` } }
            );
            thumbnail = cfRes.data.result?.thumbnail || null;
        } catch (err) {
            console.warn('Could not fetch Cloudflare video details:', err.message);
        }
        ...
        const video = await Video.create({
            ...
            title: title || 'Car Video',
            registration: registration || undefined,
            make: make || undefined,
            model: model || undefined,
            vehicleDetails: vehicleDetails ? (typeof vehicleDetails === 'string' ? JSON.parse(vehicleDetails) : vehicleDetails) : undefined,
            mileage: mileage || undefined,
            reserveCarLink: reserveCarLink || undefined,
